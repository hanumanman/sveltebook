...
export class MobileAudioPlayer {
  ...
  private setupMediaSession(title: string, artist: string) {
    if ('mediaSession' in navigator) {
      navigator.mediaSession.metadata = new MediaMetadata({
        ...
        artist: artist,
        album: 'SvelteBook',
        artwork: [{ src: '/favicon.png', sizes: '192x192', type: 'image/png' }]
      })
    }
  }
  ...
  private splitTextIntoChunks(text: string): string[] {
    ...
    console.log(
      `[splitTextIntoChunks] Text length: ${text.length}, First chunk: ${FIRST_CHUNK_LENGTH}, Other chunks: ${TARGET_CHUNK_LENGTH}`
    )
  ...
  private prefetchNextChunks() {
    ...
    itemsToPrefetch.forEach((item, index) => {
      if (!item.audioPromise) {
        console.log(
          `[prefetch] Prefetching chunk +${index + 1}:`,
          item.text.substring(0, 30) + '...'
        )
        item.audioPromise = this.fetchAudio(item.text, this.prefetchAbortController!.signal).catch(
          (err) => {
    ...
    try {
      ...
      if (item.audioPromise) {
        try {
          ...
          console.log('[playNextChunk] Prefetch failed, fetching on-demand:', errorMessage)
          this.state = 'loading'
      ...
      console.error('[playNextChunk] Error occurred:', err)
